...
// Añadimos la canción/sonido (reemplaza la URL por la de tu MP3)
const sonidoOpciones = new Audio("assets/quien-quiere-ser-millonario_ScfDHTc.mp3");
...
function inicializarGrupos() {
    ...
    cantidadGrupos = isNaN(valorInput) ? 0 : valorInput; 
    indiceGrupoActual = 0;
...
function prepararDatos() {
    ...
    while(tiposDisponibles.length < participantes_pendientes.length) {
...
function dibujarRuleta() {
    ...
    if (total === 0) {
        ...
        let mensaje = participantes_base.length > 0 ? "¡Completado!" : "No hay participantes";
        ctx.fillText(mensaje, cx, cy);
        return;
    }
...
function guardarOpciones() {
    ...
    // Le ponemos volumen al 50% por si es muy fuerte, cámbialo si quieres (de 0.0 a 1.0)
    sonidoOpciones.volume = 0.5; 
    
    // Reiniciamos el sonido por si le dan clic varias veces rápido
    sonidoOpciones.currentTime = 0; 
    
    // Reproducimos
    sonidoOpciones.play().catch(error => console.log("El navegador bloqueó el audio", error));

    // 2. Cerrar el modal como hacíamos antes
